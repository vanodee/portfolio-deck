...
**Status:** Draft v34 — About's direct-load dock auto-form (§3.3/§3.6) had a second, separate mobile-only glitch fixed: Next.js SSRs `ControlDock.tsx` ("use client") and Framer Motion bakes the `animate` prop's computed value into that server-rendered HTML, so the very first thing ever painted — server output and the client's first hydration pass, which necessarily agree — used `useBreakpoint()`'s safe "desktop" default (no `window` exists server-side to know the real viewport); harmless everywhere else in the dock (the shell is invisible for Home's entire unformed phase regardless), but About's auto-form path made the shell visible immediately, so on an actual mobile device that first paint briefly showed a tall, fully-rounded desktop-shaped capsule (height 100%, radius 999px) before snapping to the true small circle — read as "the dock starts in its vertical pill form, then becomes the small circle." Fixed by gating the reveal itself behind a plain post-mount flag (starts `false` identically on server and client — no synchronous `window` read this time, unlike an earlier attempt at this same class of bug, which itself introduced a real hydration-mismatch error caught via a console-error check and reverted same-day) that flips in the same effect-flush batch as `useBreakpoint()`'s own correction, so by the render where the shell actually becomes visible, `isMobile` is already guaranteed correct — the shell is now invisible for one imperceptible tick, then appears already in its correct small-circle geometry, no flash of either the desktop shape or a mid-animation dip; dock-formation gained a second entry path (§3.3/§3.5/§3.6): a direct/fresh load of `/about` (hard reload, typed URL, bookmark — no prior Home visit this session) now auto-plays the same shell-expand + button-stagger formation Home gets, rather than rendering already fully formed with no animation as previously specced; there's no logo-travel/FLIP phase in this path (About has no standalone logo to travel from) — after a brief `dockAboutAutoFormDelay` (220ms) rest beat, the shell goes straight into the expand + stagger, deliberate revision of the prior "direct-load renders already formed" decision (§3.6); this exposed (and fixed) a latent glitch in `dockShellTransition` (lib/dockChoreography.ts) — its shell-geometry tween used a fixed `formation`-length duration for width/height/top/radius regardless of whether `formed` was actually true, so any incidental target change during the (previously always-invisible) rest phase — e.g. `useBreakpoint()`'s own SSR-safe "desktop" guess correcting itself to "mobile" a tick after mount, which briefly points height/radius at the wrong value — played out as a visible multi-hundred-ms dip-then-recover instead of resolving instantly; that never surfaced on Home (the shell is opacity-0 for its entire unformed phase) but About's auto-form reveal makes the shell visible immediately, exposing it. Fixed generally: duration is now `0` (an instant snap) whenever `formed` is false, since the rest state is meant to be perfectly static regardless of path — only the genuine formed-flip transition uses the full `formation` duration; card-front fill reverted from per-project `previewColor` back to the fixed `card-front-bg` token (§3.2, §7 item 23), a deliberate deviation from the prior CMS-integration decision; reading-pane close button restyled to a lifting glass-button treatment, no longer matching the tag pills it was previously documented as sharing a style with (§3.13, §7 item 25); reading pane gained a net-new footer wordmark echoing the table header, and `.customSection`'s internal gap widened 24px → 32px (§3.13, §7 item 26); derived from Figma source file; About page routing/route-transition, the Chip component (from user-provided SVG exports), the Brand Card component (net-new, no Figma source), and the Photo Card / Experience Card components (from Figma, with confirmed deviations — see §3.9/§3.10) added post-Figma; About page section layout (§3.11) added post-Figma, structure/content only; Photo Card spread hover interaction (§3.9) and tool/brand grid flex-wrap layout (§3.11) added post-Figma; control dock revised (§3.3/§3.6) to persist as a single site-wide instance instead of remounting/re-forming per route; the dock's About/Back-to-Home buttons replaced with a single sliding `DockToggle` switch (§3.3/§6); control dock made responsive below 767px, restacking into a vertical column (§3.3); table grid (§4.1) now falls back from 4 to 3 columns instead of shrinking below 3-column mobile sizing, and mobile bottom padding (§4.2) increased so the stacked dock no longer covers the last card row — both found via a responsive audit, not Figma-derived; About page responsive audit follow-up (§3.8/§3.10/§3.11, §7 items 12-17) — Brand Card gains a mobile size reduction for a 2-column grid, Experience Card's text is now top-left-anchored with two responsive spread variants (narrowed fan / mobile peek-stack) replacing the original single fixed fan, the fan continuously scales to its measured available width (both axes' rotation overhang accounted for) instead of staying fixed-size, the mobile peek-stack reveals enough of each card for its full text plus a stronger stacked-card shadow, Hero gained its own 920px stacking breakpoint, and two more shrink-wrap/dock-clearance bugs (mirroring the table-grid ones above) were fixed on `/about`; AboutContent now has its own Home <-> About route-transition motion (§3.6/§6) — translates in from the right on arrival, translates out to the right before navigating back; Hero/Run/Chips/Brands now have first-visit-only section-reveal choreography (§3.11/§6), revised to deal in strictly one item at a time (no overlap) with a `.runSpreadWrap` scrollbar fix; a 404/Not Found page added (§3.3/§3.12/§6), net-new — no Figma source, not previously specced in either doc; card front's fill color and real-image cropping resolved for the Sanity CMS integration (§3.2, §7 item 23) — previewColor sources the frame fill, hotspot/focal-point cropping deferred to a plain center-crop; card front's `date` field dropped entirely for the CMS integration (§3.2, §7 item 24) — no equivalent Sanity field exists, so the micro-label is now category-only; refined once real content was flowing (§3.2, §7 item 24) — title now wraps up to 2 lines (ellipsis beyond that) respecting the same 8px padding on both sides instead of overflowing unbounded, the category micro-label bumped 7px → 11px (every real category name still fits on one line), and the card-front image source switched from `heroImage` to `previewImage` per `SCHEMA.md` §3b's intended grid/listing field, reserving `heroImage` for the expanded project page; reading pane's hero/overview/closing content shell added (§3.13), net-new for the Sanity CMS integration Phase 6 — per-project theming via 4 inline CSS custom properties (plus a 5th, local to each tool pill), structure ported from `PROJECT_PAGE_LAYOUT.md` §5/§6 with every value re-derived from this project's own tokens; reading-pane hero/closing images revised to a fixed-height crop box (§3.13) so every project renders at the same height regardless of source aspect ratio, and the category pill restacked above the title instead of inline with it; reading pane's category-specific body vocabulary added (§3.14), net-new for the Sanity CMS integration Phase 7 — 9 shared pattern components (TextImageRow, SoloTextContainer, SoloImageContainer/MediaGallery, InfoCardGrid/SoloInfoCard, PortraitImageGrid, DividerSection, LiveLinkRow) composed by all 4 category bodies, live-link responsive breakpoints (1024px/450px/320px) introduced, tag/tool pill colors re-derived from this app's own tokens rather than copied from the reference; §7 items 20/21 (real contact/resume destinations, real tool-chip logos) resolved by the CMS integration's Phase 10 (About page integration) — moved from "still open" into the resolved list; tool-chip logo rendering fixed (§3.7) — `.toolLogo`'s leftover placeholder-era background tint removed, since it showed as a visible dark patch behind any real logo asset with transparent corners; tool `Chip` gained a Brand-Card-style touch-tap reveal (§3.7, §3.8) — its wrapper is now a real `<button>`, exclusive across the grid via `AboutContent.tsx`'s `revealedToolId`, and its `.forceRevealed` CSS class was renamed `.revealed` now that it's shared by the entrance hold, mirrored real-hover, and tap-state; Home's card-grid touch-scroll gained fling momentum (§4.3) — releasing a drag used to stop dead (the WebGL canvas has to capture and forward touch manually, `touchAction: "none"`), now decays like About's native scroll instead; dock-formation's onboarding reveal (§3.3/§3.5) switched from a clip-path ellipse mask over an always-full-size pill to a real `.dockShell` layer that grows from a small circle to the full pill/column — the clipped version had no border/shadow edge of its own at the small size and read as a flat window opening rather than a solid glass shape growing; dock-formation refined (§3.3/§3.5) — button reveal now waits an extra `dockButtonRevealDelay` past the shell's own start instead of beginning the instant it does, so buttons no longer fade in before the shell has visibly grown out to reach them; mobile's border-radius morph fixed properly (§3.3/§3.5) — a shorter duration than width/height (~55% of it) was a first attempt at compensating for border-radius rendering capped at min(declared, currentHeight/2), but the real fix was the rest-state value itself: 999px (borrowed from desktop, where it's a harmless always-capped constant) was so oversized relative to the small box that the decline stayed invisible almost the whole way through regardless of duration; now starts at exactly `dockRestSize / 2` — the minimum a circle needs at that box size — leaving next to no capping headroom, so it visibly softens in step with the scale-up on the same shared duration as everything else; mobile's rest-state circle also enlarged (`dockRestSize` 56 → 64px) and gained an explicit `top` correction (`dockRestTopOffsetMobile`) so it's actually centered on the logo instead of on the shell's own uncorrected top:0 reference point
...
- **Dock formation (revised, July 2026 — real element, not a clip-path mask):** the control dock's actual layout (grid/width/position — what the traveling logo's shared-element transition needs a stable destination rect for) is real and full-size from first paint, but fully hidden (0-opacity) through onboarding; a separate glass "shell" layer behind the buttons/logo (`.dockShell`, `ControlDock.module.css`) is what's actually small — a circle roughly the dock's own height in diameter (`MOTION.onboarding.dockRestSize`), centered on where the logo will resolve (mobile needs an explicit `top` correction for this, `dockRestTopOffsetMobile` — the shell's own `top: 0` reference point is `.dock`'s padding-box edge, not the logo, which sits inset by the dock's own padding). Once the traveling logo arrives and the shell crossfades in at that small size, the shell itself grows to its full pill shape (desktop: width only, height is 100% throughout; mobile: width and height together, top-anchored so it grows downward from the logo rather than from the stack's center, plus a border-radius morph from a rest-state value of exactly `dockRestSize / 2` — the minimum a circle needs at that box size, not desktop's oversized 999px constant — down to the stacked layout's 28px rounded-rect corner, since only desktop's final shape is a true pill. border-radius renders capped at min(declared, currentHeight/2); starting from 999px (tried first) left the decline invisible until very late regardless of duration, since that value stayed above the still-growing half-height cap for almost the whole curve — starting already at the small box's own exact circle-radius leaves next to no capping headroom, so it visibly softens in step with the scale-up on the same shared duration as everything else). Each button group starts its own center-outward stagger *after* an extra delay past the shell's own start (`dockButtonRevealDelay`) — starting it at the same instant the shell began expanding left buttons fading in before the shell had actually grown out to reach them, reading as buttons floating free rather than being uncovered. Replaces an earlier clip-path-ellipse-mask version of the same idea (small ellipse → large ellipse over an already-full-size pill), which didn't carry its own border/shadow edge at the small size and read as a flat window opening rather than a solid shape growing.
...
**Revised (July 2026):** the control dock (§3.3) is persistent site-wide chrome — one component, hoisted into the root layout — and no longer animates on route change at all. Its onboarding-formation sequence (§3.5) plays exactly once per session, via one of two entry paths, and is never replayed for Home <-> About navigation once formed: on Home, the first time the deck is clicked; or, on a direct/fresh load of `/about` (see "Direct-load case" below), automatically shortly after mount. This deviates from the original design, which reused that formation choreography bidirectionally as the route-transition animation itself (dock collapse/expand + a second standalone-logo handoff on every nav) — that reuse has been removed:

- **Dock:** swaps its button content instantly, in place, the moment the route changes — Home's Visibility Toggle/Shuffle for About's Email/LinkedIn/X, and About for Back to Home. No shell collapse/expand, no button stagger, no standalone-logo handoff. Stays fully interactive throughout the navigation.
...
- **Direct-load case (revised July 2026):** a direct/fresh load of `/about` (bookmark, typed URL, hard reload — no prior Home visit this session) auto-plays the dock's formation: the shell renders visible at its small rest size immediately (no hidden/crossfade phase — there's no standalone logo to swap with), holds for a brief `dockAboutAutoFormDelay` beat (§6), then expands into the full shape with the button groups staggering in partway through — the same shell-expand + button-stagger as the Home path (§3.5), just entered directly with no logo-travel/FLIP phase at all. Detected once at mount (the dock persists across nav, so this can only be evaluated correctly at the component's true first render) and never replays: if the dock already formed via a Home -> About toggle nav earlier this session, it renders already fully formed here, same as before this revision.
...
- **Touch-tap reveal (About page polish, July 2026):** the tool variant's wrapper is a real `<button>` (was a plain `div`), so devices with no hover can still trigger the lift/label — same mechanic as Brand Card (§3.8), reused verbatim: real mouse clicks rely on hover alone; keyboard activation (`event.detail === 0`) and touch-only devices (`!matchMedia("(hover: hover) and (pointer: fine)")`) toggle a tap-controlled reveal instead. Exclusive across the grid — only one tool chip may be tap-revealed at a time, page-scoped state owned by `AboutContent.tsx` (`revealedToolId`, mirroring Brand Card's `revealedBrandId`), tapping a new chip closes whichever other one was open. The entrance "dealt in" forced reveal, a real mouse hover, and the tap-state are merged into one JS-driven `.revealed` class (`Chip.module.css`) carrying identical declarations to the `:hover` block, so every trigger looks the same.
- **Accessibility:** `aria-pressed` reflects the merged reveal state above; `aria-label` is set to the tool's name. `:focus-visible` outline matches Brand Card/`A11yCardButtons.module.css` (`2px solid rgba(255,255,255,0.9)`, 4px offset) — added so keyboard users, who have no hover equivalent, can still trigger the reveal.
...
- **Touch momentum (About page polish, July 2026):** the WebGL canvas sits on top of the scroll proxy for pointer purposes (card hover/click), which means it also has to capture touch itself (`touchAction: "none"`, `TableCanvas.tsx`) and forward deltas manually rather than letting the browser scroll natively — unlike About's plain `overflow-y` region (§3.11), which gets native momentum for free. Releasing a touch-drag used to stop dead as a result. `TableCanvas.tsx` now tracks touch velocity (EMA-smoothed across `touchmove` samples) and, on release, if it clears `MOTION.homeScrollMomentum.minFlingVelocity`, runs a decaying "fling" loop that forwards synthetic deltas through the same `onScrollForward(..., true)` path a real finger uses — so it can't drift out of sync with the rail/clamping logic already in `PlayArea.tsx`. Stops on hitting a scroll bound, decaying below `stopVelocity`, or a new touch landing (interrupts immediately, no fight with the next drag). Wheel input is untouched — it already glides via the existing `SCROLL_EASE` ease-toward-target and reads as consistent with About without needing real momentum.
...
| Dock formation | 1400ms | openEase | glass shell width/height/radius expansion (small circle → full pill/column, §3.3) + per-button stagger (90ms/button), starts right after the logo crossfade (§3.5); on a direct `/about` load (§3.6) it starts right after the auto-form delay below instead, with no logo crossfade — *placeholder, pending tuning* |
| Dock auto-form delay (direct `/about` load) | 220ms | — | rest beat before the shell auto-expands with no logo to wait behind, in place of the logoTravel+crossfade wait the Home path uses (§3.3/§3.6) — *placeholder, pending tuning* |
